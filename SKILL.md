---
name: playwright-qa-plus
description: 企業級 QA 極限稽核技能（強化版）。基於 playwright-qa-extreme 加入 SPA 實戰踩坑檢測：快取白屏、API 格式驗證、CORS 稽查、OAuth 流程、localStorage 安全、效能基準線、多語系完整性。當使用者說「QA 驗收模式」、「幫我測試這個網站」、「/qa-plus-audit」時啟動。
---
...
Phase 2    → 🤖 Claude_in_Chrome 直讀 Console/Network + Playwright 邊界攻擊
             2-A Console/Network 稽查
             2-B 表單邊界攻擊（XSS、SQL Injection、空值、超長輸入）
             2-C 500 錯誤雙重探測
             2-D SPA 快取白屏測試 ← 🆕
             2-E API Response 格式驗證 ← 🆕
             2-F CORS Header 稽查 ← 🆕
             2-G OAuth 流程驗證 ← 🆕
             2-H localStorage 安全測試 ← 🆕
             2-I Core Web Vitals 效能基準線 ← 🆕
             2-J 多語系完整性檢測 ← 🆕
             2-K crypto.randomUUID Polyfill ← 🆕
...
## Phase 2-D — SPA 快取白屏測試（Playwright 腳本）

> **來源教訓：** SPA 部署新版後，舊使用者瀏覽器快取的 JS chunk hash 已不存在 → import 失敗 → 白屏。

```js
// 測試 SPA 是否有 chunk 載入失敗的保護機制
async function testSPACacheResilience(page, url, testId) {
  // Step 1：正常載入，收集所有 JS chunk URL
  const jsChunks = [];
  page.on('response', res => {
    if (res.url().match(/\.(js|mjs)(\?|$)/) && res.status() === 200) {
      jsChunks.push(res.url());
    }
  });
  await page.goto(url, { waitUntil: 'networkidle' });

  if (jsChunks.length === 0) {
    return { id: testId, status: 'SKIP', note: '非 SPA 或無動態 chunk' };
  }

  // Step 2：攔截一個 chunk URL，模擬 404（舊 hash 已失效）
  const targetChunk = jsChunks[Math.floor(jsChunks.length / 2)];
  await page.route(targetChunk, route => route.fulfill({ status: 404, body: 'Not Found' }));

  // Step 3：重新載入，觀察是否白屏或有 retry 機制
  const errors = [];
  ...
  // Step 4：截圖 + 判定
  await shot(page, `${testId}-cache-bust`);
  const bodyText = await page.textContent('body').catch(() => '');
  const isBlank = bodyText.trim().length < 50;

  if (isBlank && errors.length > 0) {
    return { id: testId, status: 'FAIL', severity: 'P1',
      note: `JS chunk 404 後白屏，無 retry/reload 機制。錯誤：${errors[0]?.slice(0,100)}` };
  }
  return { id: testId, status: 'PASS', note: 'Chunk 失效後頁面仍可正常顯示或自動重載' };
}
```

**判定標準：**
- 白屏 + JS error → FAIL / P1（需加 `lazyWithRetry()` wrapper）
- 自動 reload 或 fallback 顯示 → PASS

---

## Phase 2-E — API Response 格式驗證（Playwright 腳本）

> **來源教訓：** Vercel 504 回傳非 JSON → 前端 `.json()` 解析失敗 → 白屏；Claude API 回應夾帶 markdown fencing → JSON.parse 失敗。

```js
// 攔截所有 API 回應，檢查 Content-Type vs 實際內容一致性
async function testAPIResponseFormats(page, url, testId) {
  const mismatches = [];
  const serverErrors = [];
  ...
  page.on('response', async res => {
    ...
    // 只檢查 API 端點（排除靜態資源）
    if (!resUrl.match(/\/api\/|\/v1\/|\/graphql/)) return;

    // 記錄所有 5xx
    if (status >= 500) {
      serverErrors.push({ url: resUrl, status });
    }

    // 檢查宣告 JSON 但實際不是 JSON
    const contentType = res.headers()['content-type'] || '';
    if (contentType.includes('application/json') && status < 500) {
      try {
        ...
        mismatches.push({
          url: resUrl, status,
          note: `宣告 JSON 但解析失敗：${e.message?.slice(0,80)}`
        });
      }
    }

    // 檢查 5xx 回傳的是不是 HTML（常見：Vercel/Nginx 504 回傳 HTML 錯誤頁）
    if (status >= 500) {
      try {
        const body = await res.text();
        if (body.includes('<!DOCTYPE') || body.includes('<html')) {
          mismatches.push({
            url: resUrl, status,
            note: `${status} 回傳 HTML 而非 JSON — 前端 .json() 會炸`
          });
        }
      } catch (_) {}
    }
  ...
  if (mismatches.length > 0 || serverErrors.length > 0) {
    return { id: testId, status: 'FAIL', severity: 'P1',
      note: `API 格式問題 ${mismatches.length} 個，5xx 錯誤 ${serverErrors.length} 個`,
      details: { mismatches, serverErrors } };
  }
  return { id: testId, status: 'PASS', note: '所有 API 回應格式正確' };
}
```

---

## Phase 2-F — CORS Header 稽查（Playwright 腳本）

> **來源教訓：** `Access-Control-Allow-Origin: *` + cookie-based auth = CSRF 風險。

```js
// 檢查所有 API 端點的 CORS 設定
async function testCORSHeaders(page, url, testId) {
  const corsIssues = [];

  page.on('response', res => {
    const resUrl = res.url();
    if (!resUrl.match(/\/api\/|\/v1\//)) return;

    const acao = res.headers()['access-control-allow-origin'];
    const acac = res.headers()['access-control-allow-credentials'];

    // 危險組合：允許所有來源 + 允許攜帶 cookie
    if (acao === '*' && acac === 'true') {
      corsIssues.push({ url: resUrl, severity: 'P0',
        note: 'CORS * + credentials=true → CSRF 漏洞' });
    }
    // 警告：寬鬆 CORS
    else if (acao === '*') {
      corsIssues.push({ url: resUrl, severity: 'P2',
        note: 'CORS * — 建議改為白名單' });
    }
  });
  ...
  if (corsIssues.some(i => i.severity === 'P0')) {
    return { id: testId, status: 'FAIL', severity: 'P0',
      note: `CORS 安全漏洞：${corsIssues.filter(i=>i.severity==='P0').length} 個端點`,
      details: corsIssues };
  }
  if (corsIssues.length > 0) {
    return { id: testId, status: 'WARN', severity: 'P2',
      note: `CORS 寬鬆警告：${corsIssues.length} 個端點`, details: corsIssues };
  }
  return { id: testId, status: 'PASS', note: 'CORS 設定安全' };
}
```

---

## Phase 2-G — OAuth 流程驗證（Claude_in_Chrome + Playwright）

> **來源教訓：** LINE/FB/IG in-app browser 擋 Google OAuth；短 token → 長 token 轉換失敗。

### 測試項目

```
1. Google OAuth Redirect 測試
   → 點擊「Google 登入」→ 驗證 redirect URL 正確
   → 回調後驗證 session/token 有效
   → 檢查是否有 state 參數防 CSRF

2. In-App Browser 偵測
   → 用 Claude_in_Chrome 檢查頁面是否有 in-app browser 偵測邏輯
   → read_page 搜尋：openExternalBrowser、googlechromes://、intent://
   → 沒有 → 標記 P2 警告

3. Token 過期處理
   → 模擬帶著過期 token 訪問受保護頁面
   → 預期：自動跳轉登入頁或 refresh token
   → 實際停在白屏或 500 → FAIL
```

**Claude_in_Chrome 檢查：**
```
→ read_page(filter="all") 搜尋以下關鍵字：
  - "openExternalBrowser" → LINE in-app browser 處理
  - "googlechromes://" → iOS Chrome 跳轉
  - "intent://" → Android 外部瀏覽器
  - "magic link" / "email link" → OAuth fallback
→ 若全部缺失且有 Google OAuth → 標記 P2
```

---

## Phase 2-H — localStorage / sessionStorage 安全測試（Playwright 腳本）

> **來源教訓：** localStorage 在 iframe / 嚴格隱私模式下被阻擋 → SecurityError → 整個 app crash。

```js
// 測試 localStorage 被 block 時 app 是否 crash
async function testLocalStorageResilience(page, url, testId) {
  // 模擬 localStorage 被禁用
  await page.addInitScript(() => {
    const throwSecurityError = () => { throw new DOMException('Access is denied', 'SecurityError'); };
    Object.defineProperty(window, 'localStorage', {
      get: throwSecurityError, configurable: true
    });
  ...
  await page.goto(url, { waitUntil: 'networkidle', timeout: 15000 }).catch(() => {});
  await page.waitForTimeout(3000);
  await shot(page, `${testId}-no-localstorage`);

  const bodyText = await page.textContent('body').catch(() => '');
  const isBlank = bodyText.trim().length < 50;
  const hasSecurityError = errors.some(e => e.includes('SecurityError') || e.includes('Access is denied'));

  if (isBlank || hasSecurityError) {
    return { id: testId, status: 'FAIL', severity: 'P1',
      note: `localStorage 被阻擋時 app crash。需要 try-catch wrapper（safeGetItem/safeSetItem）` };
  }
  return { id: testId, status: 'PASS', note: 'localStorage 不可用時 app 仍可正常運作' };
}
```

---

## Phase 2-I — Core Web Vitals 效能基準線（Playwright 腳本）

> **來源教訓：** Supabase 自訂網域冷啟動慢 3 秒；API 回應時間未監控。

```js
// 測量關鍵效能指標
async function testPerformanceBaseline(page, url, testId) {
  const results = [];
  ...
  // 2. Core Web Vitals（透過 PerformanceObserver）
  const vitals = await page.evaluate(() => {
    return new Promise(resolve => {
      ...
      new PerformanceObserver(list => {
        ...
        data.lcp = entries[entries.length - 1]?.startTime || 0;
  ...
  // 3. API 回應時間（攔截前 10 個 API 呼叫）
  const apiTimes = [];
  page.on('response', res => {
    if (res.url().match(/\/api\//) && apiTimes.length < 10) {
      const timing = res.request().timing();
      if (timing) apiTimes.push({ url: res.url().slice(-60), ms: Math.round(timing.responseEnd) });
    }
  });
  await page.reload({ waitUntil: 'networkidle' });
  await page.waitForTimeout(2000);
  ...
  // 4. 判定
  const issues = [];
  if (loadTime > 5000) issues.push(`頁面載入 ${loadTime}ms（目標 < 3000ms）`);
  if (vitals.lcp > 2500) issues.push(`LCP ${Math.round(vitals.lcp)}ms（目標 < 2500ms）`);
  if (vitals.cls > 0.1) issues.push(`CLS ${vitals.cls.toFixed(3)}（目標 < 0.1）`);
  const slowAPIs = apiTimes.filter(a => a.ms > 3000);
  if (slowAPIs.length > 0) issues.push(`${slowAPIs.length} 個 API > 3s`);

  await shot(page, `${testId}-performance`);

  if (issues.length > 0) {
    return { id: testId, status: 'WARN', severity: 'P2',
      note: issues.join('；'),
      details: { loadTime, vitals, apiTimes } };
  }
  return { id: testId, status: 'PASS',
    note: `載入 ${loadTime}ms, LCP ${Math.round(vitals.lcp || 0)}ms, CLS ${(vitals.cls || 0).toFixed(3)}`,
    details: { loadTime, vitals, apiTimes } };
}
```
...
## Phase 2-J — 多語系完整性檢測（Playwright 腳本）

> **來源教訓：** 多語系切換後出現未翻譯的 raw key（如 `nav.features`）或混合語言。

```js
// 檢測多語系切換後是否有未翻譯的 key
async function testI18nCompleteness(page, url, testId) {
  // Step 1：偵測語言切換器
  await page.goto(url, { waitUntil: 'networkidle' });
  const langSelector = await page.$('[data-lang], [class*="lang"], select[name*="lang"], button[aria-label*="語言"], button[aria-label*="language"]');
  if (!langSelector) {
    return { id: testId, status: 'SKIP', note: '未偵測到語言切換器' };
  }
  ...
  const langOptions = await page.evaluate(() => {
    ...
    if (sel) return Array.from(sel.options).map(o => o.value);
    const btns = document.querySelectorAll('[data-lang]');
    if (btns.length) return Array.from(btns).map(b => b.dataset.lang);
    return [];
  });

  // Step 3：逐一切換語言，截圖 + 檢查 raw key
  for (const lang of langOptions) {
    try {
      // 嘗試切換語言
      const sel = await page.$(`select[name*="lang"]`);
      if (sel) {
        await sel.selectOption(lang);
      } else {
        const btn = await page.$(`[data-lang="${lang}"]`);
        if (btn) await btn.click();
      }
      await page.waitForTimeout(1500);
      await shot(page, `${testId}-lang-${lang}`);

      // 檢查 raw key 模式（如 nav.features、hero.title、pricing.monthly）
      const bodyText = await page.textContent('body');
      const rawKeyPattern = /\b[a-z]+\.[a-z_]+\.[a-z_]+\b/g;
      const matches = bodyText.match(rawKeyPattern) || [];
      // 過濾掉 URL 和常見誤判
      const realKeys = matches.filter(m =>
        !m.includes('http') && !m.includes('www') && !m.includes('.com') &&
        !m.includes('.js') && !m.includes('.css') && !m.includes('.png')
      );
      if (realKeys.length > 0) {
        issues.push({ lang, keys: realKeys.slice(0, 5) });
      }
    } catch (e) {
      issues.push({ lang, error: e.message?.slice(0, 80) });
    }
  }

  if (issues.length > 0) {
    return { id: testId, status: 'FAIL', severity: 'P2',
      note: `${issues.length} 種語言有未翻譯 key`,
      details: issues };
  }
  return { id: testId, status: 'PASS',
    note: `${langOptions.length} 種語言全部通過翻譯完整性檢查` };
}
```

---

## Phase 2-K — crypto.randomUUID Polyfill 檢測（Playwright 腳本）

> **來源教訓：** iOS < 15.4 不支援 crypto.randomUUID → app 直接 crash。

```js
// 模擬舊版瀏覽器不支援 crypto.randomUUID
async function testCryptoPolyfill(page, url, testId) {
  await page.addInitScript(() => {
    if (crypto.randomUUID) {
      delete crypto.randomUUID; // 模擬舊瀏覽器
    }
  });
  ...
  const uuidErrors = errors.filter(e =>
    e.includes('randomUUID') || e.includes('is not a function')
  );

  if (uuidErrors.length > 0) {
    return { id: testId, status: 'FAIL', severity: 'P2',
      note: 'crypto.randomUUID 不存在時 app 崩潰，需 polyfill' };
  }
  return { id: testId, status: 'PASS', note: 'crypto.randomUUID 缺失時有 fallback' };
}
```
